        ...
        <form action="{{ route('admin.projects.update', ['project' => $project->slug]) }}" method="POST"
            ...
            <div class="mb-3">
                ...
                <input type="file"
                    ...
                    id="cover_image" name="cover_image" value="">
                @error('cover_image')
                    <p class="invalid-feedback">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="type_id" id="type">
                    <option @selected(!old('type_id', $project->type_id)) value="">No type selected</option>
                    ...
                    @foreach ($technologies as $technology)
                        <div class="col">
                            <input type="checkbox" class="btn-check" id="technology_{{ $technology->id }}"
                                ...
                                @checked($errors->any() ? in_array($technology->id, old('technologies', [])) : $project->technologies->contains($technology)) autocomplete="off">
            ...
            <div>
                @if($project->cover_image)
                    <img src="{{ asset('storage/' . $project->cover_image) }}" alt="Old Cover Image">
                @else
                    <span>No old cover image available</span>
                @endif
            </div>
            ...
            {{-- btns --}}
            @include('admin.projects.partials.submit-btn'){{-- back btn --}}
            @include('admin.projects.partials.back-btn'){{-- back btn --}}
            @include('admin.projects.partials.reset-btn') {{-- reset btn --}}
